...
- (void)showCoupon:(CDVInvokedUrlCommand*)command {
    NSString* url = [[command.arguments objectAtIndex:0] objectForKey:@"url"];
    MainViewController *mainView = [MainViewController current];
    [mainView showCouponView:true widthUrl:url];
}

-(void)couponNetwork:(CDVInvokedUrlCommand*)command {
    BOOL status =[[[command.arguments objectAtIndex:0] objectForKey:@"status"] boolValue];
    MainViewController *mainView = [MainViewController current];
    [mainView toggleAlertNetworkCoupon:status];
}

//Show or hide map
- (void)mapController:(CDVInvokedUrlCommand*)command {
    MainViewController *mainView = [MainViewController current];
    [mainView showCouponView:false widthUrl:nil];
